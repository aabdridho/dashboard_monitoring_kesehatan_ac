  ...
  React.useEffect(() => {
    if (acData && indoorData && kwhData && outdoorData) {
      ...
      const combinedReading: SensorReading = {
        ...
        voltage: pickValue(kwhData, ["Tegangan", "Voltage", "voltage", "V"]),
        current: pickValue(kwhData, ["Arus", "Current", "current", "A"]),
        power: pickValue(kwhData, ["Daya", "Power", "power", "W"]),
        energy: pickValue(kwhData, ["Energi", "Energy", "energy", "kWh"]),
        power_factor: pickValue(kwhData, ["PowerFactor", "powerFactor", "power_factor", "PF"]),
        status: finalStatus,
        timestamp: Date.now(),
      };
